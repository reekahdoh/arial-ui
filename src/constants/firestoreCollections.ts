/** Central collection names for Firestore (single source of truth). */
export const FIRESTORE_COLLECTION_USER_PROFILES = 'userProfiles';
export const FIRESTORE_COLLECTION_LOGIN_NAMES = 'loginNames';

// App domain collections
export const FIRESTORE_COLLECTION_RISK_ASSESSMENTS = 'RiskAssessment';
export const FIRESTORE_COLLECTION_DOMAINS = 'Domain';
export const FIRESTORE_COLLECTION_AUTHORATIVE_REFERENCES = 'AuthorativeReference';
